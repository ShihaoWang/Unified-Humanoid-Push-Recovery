...
void Nodes_Optimization_ObjNConstraint(std::vector<double> &Opt_Seed, std::vector<double> &ObjNConstraint_Val, std::vector<double> &ObjNConstraint_Type)
{
	...
	double T, dt, KE_i;		dlib::matrix<double> StateNDot_Coeff, Ctrl_Coeff, Contact_Force_Coeff; int Self_Opt_Flag;
	...
	// ObjNConst initialization for the objective function
	ObjNConstraint_Val.push_back(0);							ObjNConstraint_Type.push_back(1);
	// Make sure that the time duration is positive and has an upper bound
	ObjNConstraint_Val.push_back(T);							ObjNConstraint_Type.push_back(1);
	ObjNConstraint_Val.push_back(1.0* (Grids-1) - T);			ObjNConstraint_Type.push_back(1);

	KE_i = Node_i.KE;
	KE_tot.push_back(KE_i);

	for (int i = 0; i < Grids-1; i++) {
		Pos_Vel_Acc_VelfromPos_fromStateNdot_Coeff(dt, StateNDot_Coeff, i, 0, Robot_Config_init, Robot_Vel_init, Robot_Acc_init, Robot_VelfromPos_init);
		CtrlNContact_ForcefromCtrlNContact_Force_Coeff(Ctrl_Coeff,Contact_Force_Coeff, i, 0, Ctrl_init,  Contact_Force_init);

		Pos_Vel_Acc_VelfromPos_fromStateNdot_Coeff(dt, StateNDot_Coeff, i, 1, Robot_Config_end, Robot_Vel_end, Robot_Acc_end, Robot_VelfromPos_end);
		CtrlNContact_ForcefromCtrlNContact_Force_Coeff(Ctrl_Coeff,Contact_Force_Coeff, i, 1, Ctrl_end,  Contact_Force_end);

		// cout<<"Ctrl_init"<<endl;
		// cout<<Ctrl_init<<endl;
		// cout<<"Contact_Force_init"<<endl;
		// cout<<Contact_Force_init<<endl;
		//
		// cout<<"Ctrl_end"<<endl;
		// cout<<Ctrl_end<<endl;
		// cout<<"Contact_Force_end"<<endl;
		// cout<<Contact_Force_end<<endl;
		...
		// Real_ObjNConstraint_Stage(Node_i, Node_i_child, sigma_trans, dt, Robot_Config_init, Robot_Vel_init, Robot_Acc_init, Ctrl_init, Contact_Force_init, ObjNConstraint_Val, ObjNConstraint_Type);
		// if (i == Grids - 2)
		// {
		// 	Real_ObjNConstraint_Stage(Node_i, Node_i_child, sigma_goal, dt, Robot_Config_end, Robot_Vel_end, Robot_Acc_end, Ctrl_end, Contact_Force_end, ObjNConstraint_Val, ObjNConstraint_Type);
		// }
...
void Real_ObjNConstraint_Stage(Tree_Node &Node_i, Tree_Node &Node_i_child, std::vector<double>& sigma, double dt, std::vector<double> &Pos, std::vector<double> &Vel, dlib::matrix<double> &Acc, dlib::matrix<double> &Ctrl, dlib::matrix<double> &Contact_Force, std::vector<double> &ObjNConstraint_Val, std::vector<double> &ObjNConstraint_Type)
{
...
{	// This function will generate the spline coefficients needed for the further optimization
	double T = 4.5; double dt = T/(Grids - 1) * 1.0;
	...
	{	for (int j = 0; j < StateNDot_len/2; j++){
			...
			CubicSpline_Coeff_Pos_vec = CubicSpline_Coeff_fn(dt, x_init, x_end, xdot_init, xdot_end); // 4 by 1 vector: a, b, c, d
			// Velocity second
			PVA_init = CubicSpline_PosVelAcc4(dt, CubicSpline_Coeff_Pos_vec[0], CubicSpline_Coeff_Pos_vec[1], CubicSpline_Coeff_Pos_vec[2], CubicSpline_Coeff_Pos_vec[3], 0);
			PVA_end  = CubicSpline_PosVelAcc4(dt, CubicSpline_Coeff_Pos_vec[0], CubicSpline_Coeff_Pos_vec[1], CubicSpline_Coeff_Pos_vec[2], CubicSpline_Coeff_Pos_vec[3], 1);
			CubicSpline_Coeff_Vel_vec = CubicSpline_Coeff_fn(dt, xdot_init,xdot_end, PVA_init[2], PVA_end[2]); // 4 by 1 vector: a, b, c, d
	...
	for (int i = 0; i < Grids-1; i++){
		Pos_Vel_Acc_VelfromPos_fromStateNdot_Coeff(dt, StateNDot_Coeff, i, 0, Robot_Pos, Robot_Vel, Robot_Acc, Robot_VelfromPos);
	...
	Pos_Vel_Acc_VelfromPos_fromStateNdot_Coeff(dt, StateNDot_Coeff, Grids-2, 1, Robot_Pos, Robot_Vel, Robot_Acc, Robot_VelfromPos);
	Robotstate_Vec_i = PosNVel2StateVec(Robot_Pos, Robot_Vel);
	Robot_StateNDot_i = StateVec2StateNDot(Robotstate_Vec_i);
	Dynamics_Matrices(Robot_StateNDot_i, D_q, B_q, C_q_qdot, Jac_Full);
	Dynamics_LHS = D_q * Robot_Acc + C_q_qdot;
...
void Pos_Vel_Acc_VelfromPos_fromStateNdot_Coeff(double dt, dlib::matrix<double> &StateNDot_Coeff, int Grid_Ind, double s, std::vector<double> &Robot_Config,  std::vector<double> &Robot_Vel, dlib::matrix<double> &Robot_Acc, std::vector<double> &Robot_VelfromPos)
{
	...
	for (int i = 0; i < 13; i++) {
		...
		PVAVP_i = CubicSpline_PosVelAcc8(dt, x_a, x_b, x_c, x_d, xdot_a, xdot_b, xdot_c, xdot_d, s);
...
std::vector<double> CubicSpline_PosVelAcc4(double dt, double a, double b, double c, double d, double s)
{
    ...
    double Vel = (3*a*s*s + 2*b*s + c)/dt;
    double Acc = (6*a*s+2*b)/(dt*dt);
	PVA[0] = Pos; 			PVA[1] = Vel; 			PVA[2] = Acc;
	return PVA;
}
std::vector<double> CubicSpline_PosVelAcc8(double dt, double x_a, double x_b, double x_c, double x_d, double xdot_a, double xdot_b, double xdot_c, double xdot_d, double s)
{
	std::vector<double> PVAVP(4);
	double Pos = x_a*s*s*s + x_b*s*s + x_c*s + x_d;
	double Vel =  xdot_a*s*s*s + xdot_b*s*s + xdot_c*s + xdot_d;
	double Acc = (3*xdot_a*s*s + 2*xdot_b*s + xdot_c)/dt;
	double VelfromPos = (3*x_a*s*s + 2*x_b*s + x_c)/dt;
	PVAVP[0] = Pos;				PVAVP[1] = Vel;				PVAVP[2] = Acc;				PVAVP[3] = VelfromPos;
	return PVAVP;
}
std::vector<double> CubicSpline_Coeff_fn(double dt, double x_init, double x_end, double xdot_init, double xdot_end)
{	// This function is used to calcualte the coefficients for the cubic spline
	...
	double a, b, c, d;
	a = 2*x_init - 2*x_end + dt*xdot_end + dt*xdot_init;
  	b = 3*x_end - 3*x_init - dt*xdot_end - 2*dt*xdot_init;
    c = dt*xdot_init;
    d = x_init;
